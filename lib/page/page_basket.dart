            ...
            stream: FirebaseAuth.instance.currentUser().asStream(),
                builder: (context, currentUser) {
                  ...
                  return StreamBuilder(
                      stream:
                          Firestore.instance.collection('Basket'+user).snapshots(),
                        ...
                        if (snapshot.data == null || snapshot.data.documents ==null)
                          return CircularProgressIndicator();
                            ...
                            itemBuilder: (BuildContext context, int index) {
                              Sum+=snapshot.data.documents[index]['price'];
                              return new Card(
                                              ...
                                              decoration: BoxDecoration(
                                                image: DecorationImage(
                                                    image: NetworkImage(snapshot
                                                                        .data
                                                                        .documents[index]['cover'] == null ? CircularProgressIndicator() : snapshot
                                                                .data.documents[index]['cover']),
                                                    fit: BoxFit.cover),
                                              ),
                                            ),
                                          ),
                                ...
                                builder: (context, snapshot) {
                                  ...
                                  if (snapshot.data == null || snapshot.data.documents == null)
                                    ...
                                    for(int i=0;i<counter;i++){
                                      SumPriceInt += snapshot.data.documents[i]['price'];
                                    }
                                    SumPrice = SumPriceInt.toString();

                                  if(Sum != 0)  return RaisedButton(
